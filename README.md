#### CW-Notify-Storage
###### Летопись игровых событий и разработки
`Сентябрь 2018 года` [Auction Notify](https://github.com/evolvestin/CW-Auction-Notify) как бот, оказался крайне популярен.
Идея, которая отражена в названии была простой и очень полезной, отправлять сообщения о новых лотах людям, которые на них подписаны.
...
1. Нужно было спарсить уже имеющиеся лоты в игре. А биржа на ChatWars 3 появилась в мае месяце и там уже было 40 тысяч лотов.
...
Ну и тут сказалась моя неопытность. Тогда версии ботов немного отличались и приходилось очень много чего доделывать на ходу, чтобы
...
---
#### Где-то здесь стартует начало коммитов в этот репозиторий
`15.12.2018` - Начало больших изменений в боте. При этом старая версия продолжает работу, я сел переписывать, но путь был
долгим.

Для начала я решил парсить лоты по новому, все еще используя для этого паттерны, но сохраняя при этом гораздо болььше инфы 
о лоте. Сохранялось при этом (записывалось все это через слеш в одну ячейку и в один столбец):
...
![2018 год](screenshots/screenshot-4.png)
   ...
   > делалось это для того, чтобы я мог точно знать какой длины будет массив, когда я разделю строку по слешу.
...
Тогда мне что-то стало скучно и я забросил разработку на месяцок.
...
`24.01.2019` - Я решил добавить две проверки, которые бы парсили заново аукцион и проверяли существуют ли записи в таблице.

Функция `checker()` шла в обратном направлении (на уменьшение постов на аукционе) и проверяло существуют ли записи, если нет
писала об этом мне личные сообщения.

Функция `double_checker()` проверяла, нет ли повторящихся записей в гугл таблице.
...
для быстрого запуска главного бота [Auction Notify](https://github.com/evolvestin/CW-Auction-Notify) занимался как раз этот бот.
   > Потому что мой алгоритм, который я использовал для записи, обновления и удаления оставлял желать лучшего.
...
один раз прописать на сервере, давали возможность полностью сделать код идентичным полностью и не заменять переменные в файлике.
...
`10.11.2020` - Я решил обновлять безопасность всех моих ботов. Теперь все токены были реворкнуты 
(если посмотреть по комитам, то они мелькали частенько прямо в коде). Ревокнуты токены сервисных аккаунтов гугл,
файлы с этими токенами теперь генерируются при запуске бота, а сами они находятся теперь в переменных окружения.
